...
vector<Masina> Service::filter(const string& what, int whichFilter) const
{
	...
	v1 = repo.getAll();
	if (whichFilter == 1) {
		copy_if(v1.begin(), v1.end(), back_inserter(v2), [&](const Masina& m) {return m.getProducator()._Starts_with(what); });
	}
	else if (whichFilter == 2) {
		copy_if(v1.begin(), v1.end(), back_inserter(v2), [&](const Masina& m) {return m.getTip()._Starts_with(what); });
	}
	return v2;
