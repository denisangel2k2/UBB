...
void Tests::testServFilter()
{
	...
	string what = "pap";
	assert(repo.size() == 1);
	DynamicVector<Masina>v = serv.filter(what, 1);
...
void Tests::testServSort()
{
	...
	DynamicVector<Masina>v = serv.sort([](const Masina& m1, const Masina& m2) {if (m1.getNrInmatriculare() < m2.getNrInmatriculare()) return true; else return false; });
	assert(v.size() == 2);
...
void Tests::testServGetAll()
{
	...
	serv.srv_add(id, nrI, prod, model, tip);
	DynamicVector<Masina>v = serv.srv_getAll();
	assert(v.size() == 1);
