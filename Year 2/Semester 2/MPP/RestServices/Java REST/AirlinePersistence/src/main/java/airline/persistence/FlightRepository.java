...
public class FlightRepository implements IFlightRepository {
    ...
    public Flight findById(int id) {
        logger.traceEntry();
        Connection connection= dbUtils.getConnection();
        try(PreparedStatement ps=connection.prepareStatement("select * from flights where flight_id=?")){
            ps.setInt(1,id);
            try(ResultSet resultSet=ps.executeQuery()){
               resultSet.next();
    ...
    public void delete(int flightId) {
        logger.traceEntry();
        Connection connection=dbUtils.getConnection();
        try(PreparedStatement ps=connection.prepareStatement("delete from flights where flight_id=?")) {
            ps.setInt(1,flightId);
            ps.executeUpdate();
        }
        catch (SQLException ex){
